...
TF_2 = tf.version.VERSION.startswith("2")
...
def test_graph_lookup():
    with tf.Graph().as_default(), Session() as sess:
        ...
        if TF_2:
            assert ber_no_shape.shape[0] is None
        else:
        ...
        assert ber_list_no_shape.shape[0] == Dimension(1)
        if TF_2:
            assert ber_list_no_shape.shape[1] is None
        else:
...
def test_graph_lookup_masked():
    with tf.Graph().as_default(), Session() as sess:
        ...
        tuebingen_masked = ff_tf.ff_lookup(embeddings, "Tübingen", mask_empty_string=False, mask_failed_lookup=True,
                                           embedding_len=100)
        empty_masked = ff_tf.ff_lookup(embeddings, "", mask_empty_string=True, mask_failed_lookup=False,
                                       embedding_len=100)
